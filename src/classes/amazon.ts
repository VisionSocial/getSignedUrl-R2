...
export default class Amazon implements IAmazonClass {
  ...
  public getUrl: stringFilePromiseUrl = async (
    mimetype: string,
    fileData: IFileData
  ) => {
  ...
  public copyObject: stringFilePromiseUrl = async (
    from: string,
    fileData: IFileData
  ) => {
    let response: IS3Response;
  ...
  public deleteObjects: deleteFilesPromiseBooelan = async (
    files: Array<IS3KeyObject>
  ) => {
    let response: IS3Response;
    try {
